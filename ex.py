...
plt.pcolor(user_product_star)
plt.xticks(np.arange(0.5, len(user_product_star.columns), 1), user_product_star.columns)
plt.yticks(np.arange(0.5, len(user_product_star.index), 1), user_product_star.index)
plt.title('Heatmap by plt.pcolor()', fontsize=20)
plt.xlabel('productId', fontsize=14)
plt.ylabel('userId', fontsize=14)
plt.colorbar()
...
i = int(input())
print(recomm_product_by_userid(i))